...
@interface JASCircleProgressView ()

@property (nonatomic,strong) JASCircleShapeLayer *progressLayer;
...
@implementation JASCircleProgressView
...
- (void)layoutSubviews {
    ...
    self.progressLabel.center = fPoint(self.center.x - self.frame.origin.x, self.center.y - self.frame.origin.y);
}
...
- (UILabel *)progressLabel {
    if (!_progressLabel) {
        ...
        [self addSubview:_progressLabel = progressLabel];
    }
...
- (void)setupViews {
    ...
    // Add Progress Layer
    self.progressLayer = [[JASCircleShapeLayer alloc] init];
    self.progressLayer.frame = self.bounds;
