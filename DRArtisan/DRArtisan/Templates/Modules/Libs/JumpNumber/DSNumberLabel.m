...
@property (nonatomic, retain) NSMutableArray *numberPoints;// 记录每次UILabel更改值的间隔时间及输出值。
...
- (void)changeNumberBySelector {
    if (_indexNumber >= kPointsNumber) {
        self.text = [NSString stringWithFormat:@"%.1f",_endNumber];
        return;
    } else {
